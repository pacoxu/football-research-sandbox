...
[![deploy-pages](https://github.com/pacoxu/football-research-sandbox/actions/workflows/deploy-pages.yml/badge.svg)](https://github.com/pacoxu/football-research-sandbox/actions/workflows/deploy-pages.yml)
[![validate-data](https://github.com/pacoxu/football-research-sandbox/actions/workflows/validate-data.yml/badge.svg)](https://github.com/pacoxu/football-research-sandbox/actions/workflows/validate-data.yml)
...
当前状态：研究样本库，最近维护日期 `2026-07-19`。本站不是中国足协、AFC、FIFA、俱乐部或任何第三方平台的官方数据库；页面里的球员、留洋和教练数量只代表本仓库已经结构化并通过当前来源口径纳入的样本，不代表官方全量人数。

当前结构化基线：459 名球员、20 项赛事快照、9 个专题项目，覆盖 8 个国家/地区。
...
- 单页数据中心集中展示覆盖质量、五国青训路径比较、项目目录与教练目录
- 国内青训教练独立页面，当前覆盖校园、足校、职业梯队、基地、地区体校和民间项目 13 位具名样本
- 2030—2042 亚洲世界杯长期预测，公开名额假设、模型权重、回测和不确定区间
- 本地 `JSON + SQLite` 双轨存储
...
│   │   ├── projects.json
│   │   └── tournaments.json
│   ├── schema/              # Draft 2020-12 JSON Schema 与校验 manifest
│   └── site/                # 前端聚合 JSON；meta.json 公开构建与质量元信息
├── docs/                    # 研究说明、收集口径和待核清单
...
- `data/raw/china-youth-development-coaches.json` 维护国内具名青训教练、机构类型、年龄段、任期/赛季快照和逐条来源；`coaches.html` 负责前端展示与筛选。
