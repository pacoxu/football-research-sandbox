...
> 实施状态：issue #9 已按本审计边界完成。10 个新增组合各录入 23 人，并与既有澳大利亚 U17/U20、乌兹别克 U17 合并为 13 个完整组合；伊朗 U23 2024、U17 2026 以 `not-applicable` 结构化记录。
...
- `data/raw/afc-comparison-rosters.json` 保存 230 行经逐行核对的 AFC 终报名原始转录，`scripts/generate-afc-comparison-rosters.mjs` 负责稳定 ID 去重、球员记录生成、乌兹别克 U17 契约统一和覆盖元数据更新。
- 230 条新增赛事参与记录归并为 207 名球员；同名、同生日且姓名词序互换的跨年龄记录复用同一 ID，不重复建档。
- 每条新增参与记录均为 `registered` / `final-squad` 并保留官方号码；伊朗 U23 2026 的缺 2 号与两个 22 号原样保留并写入说明。
- AFC 俱乐部字段仅作为带日期的 `tournament-snapshot`。Aspire 等国家级培养机构可使用 `national-academy`，但只有官方报名组织明确对应时才标注。
- AFC 拉丁转写保留在 `name` / `local_name`；没有足协或俱乐部官方来源时不推测波斯文、阿拉伯文或西里尔文。赛事代表身份不用于推断出生地、入籍方式或归化状态。
