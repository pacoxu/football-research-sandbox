...
外部来源的文字、图片、名单、数据库字段、商标和媒体仍归原权利方所有。仓库的 [LICENSE](../LICENSE) 只将 MIT License 用于代码，不会把研究数据、生成产物或第三方内容重新授权为开放数据。

引用资料时应尽量摘要化、结构化并保留原始链接、核验说明和 provenance 字段。任何进一步复用或发布都需要自行遵守原始来源条款、数据库权利、隐私与人格权、商标规则、平台条款和适用法律。
