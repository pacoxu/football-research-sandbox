...
| 字段 | 含义 |
| --- | --- |
| `systems[]` | 国家级体系条目，当前为中国、日本、韩国、挪威、丹麦、瑞典。中国条目把历史预备队联赛、现行U21联赛和成年B队通道拆开记录。 |
...
七个重点青训专题采用 `schema_version: 2`。`people[]` 保存专题内唯一人物，`roster_views[].members[]` 只保存人物与批次的关系；其他旧专题仍可保留内嵌 `players[]`。

| 字段 | 含义 |
| --- | --- |
| `people[].id` | 专题内稳定人物 ID；跨批次重复出现仍只保留一个人物。 |
| `people[].player_id` | 可选的主球员库 ID；仅在现有身份可可靠匹配时关联。 |
| `members[].relationship` | `development-core`、`batch-participant`、`short-training`、`tournament-only`、`partner-player`、`project-adjacent` 或 `prediction-only`。 |
| `members[].verification_status` | `verified` 或 `needs-review`；待核线索不计入已验证人数。 |
| `counting` | 批次的公开人数、已列人数、唯一人数、已核人数、待核人数、完整度和口径说明。 |
| `event_records`、`program_metrics` | 赛事结果和项目宣传/执行数字，不得伪装成人物。 |

`dossiers[].people[].current_status` 用于专题人物唯一的当前状态：

| 字段 | 含义 |
| --- | --- |
| `category` | `active-first-team`、`active-reserve`、`active-professional`、`retired`、`retired-coach`、`youth-development` 或 `needs-review`。 |
...
2002 中国成年世界杯与 1983—2005 中国 U16/U17/U20 世界赛的 `china_squad[]` 固定包含 `appearances`、`starts`、`substitute_appearances`、`minutes` 和 `goals`。`minute_status.available_fields` 与 `missing_fields` 必须完整且互斥地声明五个字段的可用性：已核实值使用非负整数，资料不足的字段必须是 `null`，不得按比赛场次、首发阵容、换人时间或标准比赛时长推算。`minute_status.status` 为 `partial` 时必须解释来源边界；只有五项均已从可靠逐场或技术报告核实时才能写 `complete`。

`minute_status.verified_totals` 保存本届中国队的可复算审计总计：`matches` 必须等于 `china_matches` 数量，其他键只能对应 `available_fields`。校验器要求逐人加总与这些值完全一致；存在完整首发数据时，首发总数还必须等于比赛数乘 11，出场总数必须等于首发与替补登场总数之和。分钟总计采用技术报告直接公布的逐人值之和，不以比赛时长倒推，因此红牌等特殊情况不会被错误填平。
...
`data/raw/asian-coaches.json` 单独维护五大联赛之外的扩展样本。一个教练实体可挂多个 `stints`，避免国家队和俱乐部经历重复建档。Issue #12 第二批加入 Kevin Muscat、Kim Pan-gon、Akira Nishino、Masatada Ishii、Amir Ghalenoei 和 Choi Kang-hee；这些记录不写入五大联赛主表。
...
`data/raw/china-youth-development-coaches.json` 与国字号青年队教练、亚洲一线队主教练分开维护。当前14人覆盖石门五小、王楚、根宝基地、东北路小学、山东泰山/鲁能足校体系、恒大足校、清华附中、中国足球小将和阿勒泰地区体校；唐晓程、崔鹏分别只按2025年U15、U17赛事快照记录，徐亮、毛剑卿只按2023年快照记录，柳忠云标为前总教练并保留持续返校指导的边界。
...
| `batch_assignments` | 民间项目按出生年份拆分的实际带队快照；每批次必须写明职责、年龄段、核查年度和独立来源，不能把创办人身份自动外推为所有批次完整教练组。 |

“现任”只在核查日仍有机构或赛事资料支持时使用。单届赛事能够确认的梯队岗位使用 `confirmed-YYYY` 或历史年份任期；没有离任公告不能反向证明持续任职。Issue #12 的国内扩展因此保留“已核样本”和 watchlist 两层，不用跨年份报道拼出一张虚假的现任全表。
...
`data/site/meta.json` 额外公开以下派生字段：

| 字段 | 含义 |
| --- | --- |
| `schema_version` | meta 结构版本；当前为 `1`。 |
| `build` | 本地为可复现的 `unstamped`；部署副本才写入 commit、构建时间和 `deployed`。 |
| `sample_notice` | 中英文样本声明，明确仓库研究样本不代表官方全量。 |
| `coverage_summary` | 数据集数、记录数、过期和待复核记录汇总。 |
| `datasets[].quality` | `complete`、`mixed`、`partial`、`needs-review`、`stale`、`excluded` 数量；过期优先于完整度。 |
| `datasets[].source_tiers` | 按现有 1—6 级来源政策汇总；不能确定时归入 `unclassified`。 |
| `datasets[].missing_fields` | 当前样本的待补字段及影响记录数。 |
| `catalog_quality` | 项目目录和教练目录按稳定 ID 发布的质量、来源等级、核查日和待补字段。 |
...
| 字段 | 含义 |
| --- | --- |
| `source` | 来源名称、运营者、官网、服务条款、等级、核查日和中英文风险说明。 |
| `records[].player_id` | 可选站内稳定球员 ID；仅用于关联，不允许反向覆盖主球员事实。 |
| `report_type` | `image-report`、`talent-of-the-day`、`player-profile` 或 `guest-report`。 |
| `source_scope` | 链接指向 `individual`、`country-index` 或 `category-index`。 |
| `potential_rating` | 来源发布时的 10 分制报告快照；未公开评分为 `null`。 |
...
世界杯预测字段、模型边界和更新方式见 `docs/world-cup-forecast.md`。`world-cup-forecast.json` 的顶层结构为 `metadata`、`methodology`、`assumptions`、`backtest`、`editions` 和 `sources`；概率使用 0—1 小数，`interval_80` 保存第 10—90 百分位。
